...
for signal_idx = 1:length(data) % wczytywanie po sygnale do analizy
    ...
    [windows, ~] = buffer(signal.signal3, window_len_samples, floor(window_len_samples * overlapp_percent / 100), 'nodelay'); % podzielenie sygnalu na okna
...
%         SE = [SE, Se];
        ...
        SignalThreeFeatures = [SignalThreeFeatures; KTEmean, KTEvar, PSDmean, PSDvar, var(SE), mean(SE), ptp, PTP_kurt, mptp, max(centroid_interpl), a(end,:), logPSD];
...
save("SignalThreeFeatures.mat", "SignalThreeFeatures");
