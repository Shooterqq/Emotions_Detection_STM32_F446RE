...
function SignalFourFeatures = SignalFourFeatures(file_path)
...
for signal_idx = 1:length(data) % wczytywanie po sygnale do analizy
    ...
    [windows, ~] = buffer(signal.signal4, window_len_samples, floor(window_len_samples * overlapp_percent / 100), 'nodelay'); % podzielenie sygnalu na okna
...
%         SE = [SE, Se];
        ...
        SignalFourFeatures = [SignalFourFeatures; KTEmean, KTEvar, PSDmean, PSDvar, var(SE), mean(SE), ptp, PTP_kurt, mptp, max(centroid_interpl), a(end,:), logPSD];
...
save("SignalFourFeatures.mat", "SignalFourFeatures");
