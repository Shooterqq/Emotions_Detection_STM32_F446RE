...
for signal_idx = 1:length(data) % wczytywanie po sygnale do analizy
    ...
    [peaks, locs] = findpeaks(signal.signal5, 'MinPeakDistance', min_distance, 'MinPeakHeight', mean(signal.signal3));
    
    % Znalezienie maksimów sygnału o zadanej szerokości szczytu 0,83 hz
    [peaks, locs] = findpeaks(signal.signal5,fs,'MinPeakWidth',0.4, 'MinPeakDistance', 1);
...
SignalFiveFeatures = RESP_FEATURES_SIGNAL;
end
