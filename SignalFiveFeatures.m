...
function SignalFiveFeatures = SignalFiveFeatures(file_path)
...
for signal_idx = 1:length(data) % wczytywanie po sygnale do analizy
    ...
    [windows, ~] = buffer(signal.signal5, window_len_samples, floor(window_len_samples * overlapp_percent / 100), 'nodelay'); % podzielenie sygnalu na okna
...
%         SE = [SE, Se];
        ...
        SignalFiveFeatures = [SignalFiveFeatures; KTEmean, KTEvar, PSDmean, PSDvar, var(SE), mean(SE), ptp, PTP_kurt, mptp, max(centroid_interpl), a(end,:), logPSD];
...
save("SignalFiveFeatures.mat", "SignalFiveFeatures");
